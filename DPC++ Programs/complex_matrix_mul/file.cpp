...
//#include "/glob/development-tools/versions/oneapi/2023.2/oneapi/dev-utilities/latest/include/dpc_common.hpp"
#include "Complex.hpp"
...
class CustomDeviceSelector : public device_selector 
{
    ...
    int operator()(const device &dev) const override {
        int device_rating = 0;
//We are querying for the custom device specific to a Vendor and if it is a GPU device we are giving the highest rating //as 3 . The second preference is given to any GPU device and the third preference is given to CPU device. 
...
// in_vect1 and in_vect2 are the vectors with N complex nubers and are inputs to the parallel function
void DpcppParallel(queue &q, std::vector<Complex2> &in_vect1, std::vector<Complex2> &in_vect2, std::vector<Complex2> &out_vect) {
...
void DpcppScalar(std::vector<Complex2> &in_vect1, std::vector<Complex2> &in_vect2, std::vector<Complex2> &out_vect) {
