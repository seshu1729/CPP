#include <CL/sycl.hpp>
using namespace cl::sycl;
using namespace std;
static const int N = 256;

int main()
{
    queue q{property::queue::in_order()};
    cout << "Device name: " << q.get_device().get_info<info::device::name>() << "\n";
    //int *data = static_cast<int *>(malloc_shared(N * sizeof(int), q));
    auto data = malloc_shared<int>(N * sizeof(int), q);
    //auto *data = malloc_shared<int>(N * sizeof(int), q); // Will work 
    //auto *data = malloc_shared(N * sizeof(int), q); // Will give an error
    
    for (int i = 0; i < N; i++)
    {
        data[i] = 10;
    }
    
    
    q.parallel_for(range<1>(N), [=](id<1> i) { data[i] += 2; });
    q.parallel_for(range<1>(N), [=](id<1> i) { data[i] += 3; });
    q.parallel_for(range<1>(N), [=](id<1> i) { data[i] += 5; });
    
    q.wait();
    for (int i = 0; i < N; i++) std::cout << data[i] << " ";
    cout << "\n";
    free(data, q);
    
    return 0;
}
